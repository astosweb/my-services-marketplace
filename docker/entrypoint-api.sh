...
chown -R gobid:gobid /app/.data 2>/dev/null || true
...
if [ "${SEED_ON_START}" = "true" ]; then
  ...
  ALLOW_SEED=true runuser -u gobid -- env ALLOW_SEED=true ./node_modules/.bin/prisma db seed \
    || ALLOW_SEED=true runuser -u gobid -- env ALLOW_SEED=true tsx prisma/seed.ts \
