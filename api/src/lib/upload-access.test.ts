...
describe("upload-access", () => {
  ...
  it("creates a verifiable signed URL", () => {
    ...
    expect(
      verifyPrivateUploadToken(key, url.searchParams.get("token") ?? undefined, url.searchParams.get("exp") ?? undefined),
    ).toBe(true);
    expect(verifyPrivateUploadToken(key, "deadbeef", url.searchParams.get("exp") ?? undefined)).toBe(
      false,
    );
  });
});
