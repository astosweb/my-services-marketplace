...
export class RealtimeClient {
  ...
  on(event: string, handler: Handler) {
    ...
    this.socket?.on(event, this.wrap(handler));
    return () => this.off(event, handler);
  }

  off(event: string, handler: Handler) {
    this.handlers.get(event)?.delete(handler);
    this.socket?.off(event, this.wrap(handler));
  }
  ...
  async connect() {
    ...
    this.connecting = (async () => {
      ...
      socket.on("connect_error", async (error) => {
        // Refresh token on auth failures then reconnect.
        if (/unauthorized|jwt|token/i.test(error.message)) {
          try {
            ...
            // Session likely expired; leave disconnected until caller reconnects.
          }
        }
      });
      ...
      socket.on("disconnect", () => {
        if (!this.intentionalClose) {
          // Socket.IO handles reconnect; presence.ping resumes after ready.
        }
      });

      for (const [event, set] of this.handlers) {
        for (const handler of set) {
          socket.on(event, this.wrap(handler));
        }
      }
