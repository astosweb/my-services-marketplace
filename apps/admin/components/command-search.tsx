...
import { searchItems as searchItemsFromConfig } from "@/config/navigation"
...
export function CommandSearch({ open, onOpenChange }: CommandSearchProps) {
  ...
  const searchItems = React.useMemo(
    () =>
      searchItemsFromConfig
        .filter(
          (item) =>
            !item.permission ||
            (!isLoading && permissions.includes(item.permission)),
        )
        .map((item) => ({
          title: item.title,
          url: item.url,
...
export function SearchTrigger({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground h-8 px-3 py-1 relative w-full justify-start text-muted-foreground sm:pr-12 md:w-36 lg:w-56"
    >
